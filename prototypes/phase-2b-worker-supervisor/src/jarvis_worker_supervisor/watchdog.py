...
class Watchdog:
    ...
    def _drain(self) -> None:
        ...
        # Final sweep to guarantee no unreaped children
        import psutil
        from jarvis_worker_supervisor.shutdown import force_terminate_process
        for instance_id, child in list(self.supervisor._children.items()):
            force_terminate_process(child.pid, 2.0)
            try:
                child.wait(timeout=2.0)
            except BaseException:
                pass
            self.supervisor._children.pop(instance_id, None)
