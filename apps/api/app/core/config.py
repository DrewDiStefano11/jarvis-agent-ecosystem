...
class Settings(BaseSettings):
    ...
    model_ollama_model: str = Field("qwen3.5:4b", alias="JARVIS_MODEL_OLLAMA_MODEL", min_length=1)
    ...
    def validate_model_provider_settings(self) -> Settings:
        ...
        for label, value in (
            ...
            if not configured or not configured <= supported_capabilities:
    ...
    def parsed_model_pricing(self) -> dict[str, dict[str, float]]:
        ...
        for model, pricing in payload.items():
            ...
            if set(pricing) != required:
                raise ValueError("model pricing entries require input and output per-million rates")
            if any(
