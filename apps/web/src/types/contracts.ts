...
export interface SystemStatus {status:string;environment:string;apiSchemaVersion:string;seedDataVersion:string;emergencyStop:boolean;simulator:{state:'idle'|'running'|'paused'|'completed'|'failed'|'recovery_required';currentStep:number;totalSteps:number;accelerated:boolean};resources:{name:string;value:string;label:string}[];lastSynchronizedAt:string;storageBackend:string;databaseHealthy:boolean;databaseRevision:string;schemaCurrent:boolean;eventSessionId:string;outboxPendingCount:number;outboxExhaustedCount:number;recoveryRequired:boolean;activeWorkflowRunId:string|null;lastCheckpointId:string|null;lastStartupAt:string|null;lastCleanShutdown:string|null}
...
export interface EventEnvelope {eventId:string;schemaVersion:string;eventType:string;timestamp:string;sequenceNumber:number;eventSessionId?:string|null;correlationId:string;taskId:string|null;agentId:string|null;source:string;payload:Record<string,unknown>}
