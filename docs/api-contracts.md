...
- Events: `WS /ws/events`
- Agent runtime: `GET /api/agent-runtime/runs`, `GET /api/agent-runtime/runs/{run_id}`, `GET /api/agent-runtime/runs/{run_id}/{events|attempts|checkpoints|lineage}`, and `POST /api/agent-runtime/commands`
...
Agent-runtime routes use the same successful-response envelope as the rest of the API: the typed runtime payload is carried in `data` with `meta.schemaVersion` of `"1.0"`, and pagination fields stay inside `data`. Runtime domain failures continue to use the standard error body with their stable codes (`run_not_found` at 404, `run_already_exists`/`version_conflict`/`command_conflict` at 409) and are never wrapped in a success envelope. The command route returns the full `RuntimeCommandResult` inside `data`, preserving `idempotent_replay`, snapshot, and event fields.
...
Health resolves the runtime persistence component before computing the top-level status, and it queries runtime tables only when the database is reachable and the schema is current. Any configured runtime component that is not healthy—unavailable persistence, a repository query failure, integrity or replay corruption, a malformed health result, or exhausted runtime outbox rows—degrades the top-level status. Unexpected runtime repository errors stay bounded as `runtime_health_query_failed` and invalid result shapes as `runtime_health_invalid_response`; neither exposes exception text, SQL, database paths, or tracebacks. A stale schema (`schema_stale`) or unreachable database (`database_unreachable`) keeps its existing degradation result instead of being reported as a runtime query failure.
