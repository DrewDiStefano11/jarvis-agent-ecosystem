...
This first live network includes six authored stations and six directed routes: Models desk A ↔ B (`POSITION_022` / `POSITION_028`), Models review desk A ↔ B (`POSITION_030` / `POSITION_032`), and Focus C desk A ↔ B (`POSITION_120` / `POSITION_121`). Each pair uses the original candidate navigation engine and collider tests. All cross-room travel remains unavailable. The full candidate overlay remains explicitly unapproved because its unresolved door associations and broader geometry have not passed the original promotion lifecycle.
...
- `scripts/vendor/office-navigation.ts` preserves `src/office/floor1/navigation/candidateNavigation.ts` byte for byte (LF SHA-256 `abff5ee7af915d83eb246363ad23c3cce80ae402251e4b8bcd8c1f6eedbc660f`). The generator binds the unchanged source dimensions and verifies its checksum before use.
...
Run `node scripts/build-office-network.cjs` after frontend dependency installation to reproduce the six valid routes, four representative blocked/restricted route probes, 260 colliders, and 47 door policies. It rejects unexpected geometry or vendor changes. Use `--write` only when deliberately regenerating and reviewing the catalog. Numeric registration can independently be reproduced with optional Pillow/numpy/pypdf installed:
