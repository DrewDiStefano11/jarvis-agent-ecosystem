...
Durability regressions also simulate orphaned idempotency leases with concurrent reclamation, pause/emergency-stop during post-commit WebSocket publication, and reset session rotation followed by the first new-session command. Tests assert single execution, non-regressing checkpoints, unique workflow-step audits, and monotonic per-session sequence pairs at the database level.
