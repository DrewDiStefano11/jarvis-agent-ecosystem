...
Durability regressions also simulate orphaned idempotency leases with concurrent reclamation, pause/emergency-stop during post-commit WebSocket publication, reset session rotation followed by the first new-session command, repeated restarts of recovery-required work, and clean restart after completion. Tests assert single execution, non-regressing checkpoints, unique workflow-step audits, monotonic per-session sequence pairs at the database level, and the reset-required terminal-state guard after restart. Health tests cover both an unreachable probe result and an independently detected stale Alembic revision.
