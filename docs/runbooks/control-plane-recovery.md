...
* **Restoration:** If the file is missing or path invalid, verify `JARVIS_DATABASE_URL` absolute path (remembering `sqlite:////` for Windows absolute paths).
...
* **Recovery:**
  ...
  5. **Prohibited:** Never delete pending outbox rows solely to clear health warnings. Let the retry ceiling handle dead letters, or fix the subscriber.
