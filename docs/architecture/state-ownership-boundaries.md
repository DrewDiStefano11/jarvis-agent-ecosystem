...
| Expired lease | `task_leases` expiration vs DB time | Task-lease service (sweeper) | Terminate lease, requeue task | Assuming PID absence means task failed |
| Stale worker | `workers` last_seen heartbeat | Worker supervisor / Lease sweeper | Reconcile OS process / Reclaim lease | Worker killing another worker's process |
...
* **Exact outbox delivery guarantee:** Currently at-least-once, but exact behavior with disconnected browsers (who miss live WS but fetch full snapshots) requires formal modeling.
...
* **Dead-letter behavior:** Outbox retries fail permanently after a ceiling, but no formal dead-letter table exists.
