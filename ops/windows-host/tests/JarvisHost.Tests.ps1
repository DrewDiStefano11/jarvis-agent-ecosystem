Describe "JarvisHost Common Module" {
...
Describe "Config Validation" {
  It "Rejects missing file" { { Import-JarvisHostConfig -Path 'nonexistent.json' } | Should -Throw }
}
Describe "Process Ownership" {
  It "Returns false for nonexistent PID" { Test-JarvisProcessOwnership -PID 999999 -ExpectedExecutable 'fake' -ExpectedWorkingDir 'fake' | Should -Be $false }
}
